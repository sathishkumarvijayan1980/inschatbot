...
namespace Microsoft.BotBuilderSamples
{
    ...
    /// </summary>
    public class PolicyDocDialog : ComponentDialog
    {
        ...
        public PolicyDocDialog(IStatePropertyAccessor<DocumentState> userProfileStateAccessor, ILoggerFactory loggerFactory)
            : base(nameof(PolicyDocDialog))
        {
        ...
        public IStatePropertyAccessor<DocumentState> UserProfileAccessor { get; }
        ...
        private async Task<DialogTurnResult> InitializeStateStepAsync(WaterfallStepContext stepContext, CancellationToken cancellationToken)
        {
            ...
            {
                var greetingStateOpt = stepContext.Options as DocumentState;
                ...
                {
                    await UserProfileAccessor.SetAsync(stepContext.Context, new DocumentState());
                }
            }
                                                ...
                                                CancellationToken cancellationToken)
        {
            ...
            {
                return await GetPolicyDocument(stepContext);
            }
        ...
        private async Task<DialogTurnResult> GetPolicyDocument(WaterfallStepContext stepContext)
        {
            var context = stepContext.Context;
            var greetingState = await UserProfileAccessor.GetAsync(context);

            // Display their profile information and end dialog.
            await context.SendActivityAsync($"Getting policy document for {greetingState.Mobile} - {greetingState.BirthYear} from CRM...Please wait...");
                ...
                using (var stringContent = new StringContent(authJson, Encoding.UTF8, "application/json"))
                {
                    ...
                    if (authResponse.IsSuccessStatusCode)
                    {
                        ...
                        if (authKeyValue != string.Empty)
                        {
                            ...
                            {
                                ReleaseKey = "6684cb22-ab32-44e9-addf-fef96b7e7b48",
                                RobotIds = robitIDs,
                                JobsCount = 0,
                                Strategy = "Specific",
                                InputArguments = "{customer_policy_id:" + greetingState.Mobile + "}",
                            };
                                ...
                                using (var startJobContent = new StringContent(startJobJson, Encoding.UTF8, "application/json"))
                                {
                                    ...
                                    if (startJobResponse.IsSuccessStatusCode)
                                    {
                                        ...
                                        if (jobCreatedID != null)
                                        {
                                            ...
                                            {
                                                policyDocument = await GetRenewalJobStatusResult(getJobHttpClient, greetingState.Mobile, authKeyValue).ConfigureAwait(false);
                                                Thread.Sleep(5000);
                                                policyDocument = @"[Policy Document Download]("+policyDocument+")";
                                            }
                                        }
                                    }
                                }
            ...
            await context.SendActivityAsync(policyDocument);
        ...
        private static async Task<string> GetRenewalJobStatusResult(HttpClient getJobHttpClient, string referenceNumber, string authKeyValue)
        {
            ...
            if (getJobResponse.IsSuccessStatusCode)
            {
                ...
                polRenewalDate = getJobResultValues.value[0].Output.output_api;
            }
            else
            {
